...
RequirePath "path"
  = head:Identifier tails:RequirePathTail* {
    return [head.name].concat(tails);
  }

RequireParameter "parameter"
  = DefaultToken _ AsToken _ define:Identifier {
    return { type: "RequireDefaultArgument", define: define.name };
  }
  / name:Identifier _ AsToken _ define:Identifier {
    return { type: "RequireArgument", name: name.name, define: define.name };
  }
  / name:Identifier {
    return { type: "RequireArgument", name: name.name, define: name.name }
  }

RequireParameterPart "parameter"
  = "," _ parameter:RequireParameter {
    return parameter;
  }

RequireDestructParams "parameters"
  = head:RequireParameter tails:RequireParameterPart* {
    return [head].concat(tails);
  }

RequireStatement "require"
  = RequireToken _ path:RequirePath EOS {
    return { type: "RequireStatement", name: path[path.length-1], package: path };
  }
  / RequireToken _ path:RequirePath _ AsToken _ name:Identifier EOS {
    return { type: "RequireStatement", name: name.name, package: path };
  }
  / RequireToken _ destruct:RequireDestructParams _ FromToken _ path:RequirePath EOS {
    return { type: "RequireStatement", name: path[path.length-1], package: path, destruct: destruct };
  }



ImportDefaultStatement "default import"
  = define:Identifier {
    return [{ type: "ImportDefaultArgument", define: define.name }];
  }

ImportNamespaceStatement "namespace import"
  = "*" _ AsToken _ define:Identifier {
    return [{ type: "ImportNamespaceArgument", define: define.name }];
  }

ImportNamedSingleParameter "parameter"
  = DefaultToken _ AsToken _ define:Identifier {
    return { type: "ImportDefaultArgument", define: define.name };
  }
  / name:Identifier _ AsToken _ define:Identifier {
    return { type: "ImportArgument", name: name.name, define: define.name };
  }
  / name:Identifier {
    return { type: "ImportArgument", name: name.name, define: name.name };
  }

ImportNamedChunk "parameter"
  = "," _ param:ImportNamedSingleParameter {
    return param;
  }

ImportNamedList
  = head:ImportNamedSingleParameter _ tails:ImportNamedChunk* {
    return [head].concat(tails);
  }

ImportNamedStatement "named import"
  = "{" _ chunks:ImportNamedList _ "}" {
    return chunks;
  }

ImportDefaultNamespacedStatement
  = defaultImport:ImportDefaultStatement _ "," _ "*" _ AsToken _ namespaceImport:Identifier {
    return [
      { type: "ImportNamespaceArgument", define: namespaceImport.name },
      defaultImport[0]
    ];
  }

ImportDefaultNamedStatement
  = defaultImport:ImportDefaultStatement _ "," _ chunks:ImportNamedStatement {
    return [defaultImport[0]].concat(chunks);
  }


ImportStatementMiddle
 = ImportDefaultNamespacedStatement
 / ImportDefaultNamedStatement
 / ImportNamedStatement
 / ImportNamespaceStatement
 / ImportDefaultStatement

ImportStatement "import"
  = ImportToken _ stat:ImportStatementMiddle _ FromToken _ path:StringLiteral EOS {
    return { type: "ImportStatement", path: path.value, destruct: stat }
  }
  / ImportToken _ path:StringLiteral EOS {
